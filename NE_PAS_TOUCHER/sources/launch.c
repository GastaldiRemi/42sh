...
** Last update Mon May 30 15:47:35 2016 Juliani Renaud
*/
...
int		check_action(char **tab, char **env, t_plist *envlist)
{
    ...
    return (setenv(envlist, tab));
  else if (my_strcmp(tab[0], "unsetenv") == 0)
    return (unsetenv(envlist, tab));
...
int		launch(char **env, t_plist *envlist, t_pcmd *cmd, t_psep *sep)
{
  t_cmd		*tmp;
  char		*acces;
  int		i;
  ...
  while (tmp)
    {
      if (my_strcmp(check_exit(tab), "FAIL") != 0)
	{
	  if (tab[1] != NULL)
